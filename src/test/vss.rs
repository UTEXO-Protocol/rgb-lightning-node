...
mod tests {
    ...
    fn prepare_synced_kv_fixture(db_path: &Path, store_id: &str, signing_key: SecretKey) {
        ...
        local
            .write(
                crate::ldk::RGB_SENDER_FUNDING_NAMESPACE,
                "",
                "funding",
                b"old".to_vec(),
            )
    ...
    fn synced_kv_os_kill_matrix() {
        ...
        for mode in ["write", "remove"] {
            for checkpoint in trace_synced_kv_checkpoints(mode) {
                ...
                let local_value =
                    local.read(crate::ldk::RGB_SENDER_FUNDING_NAMESPACE, "", "funding");
                let remote_value =
                    remote.read(crate::ldk::RGB_SENDER_FUNDING_NAMESPACE, "", "funding");
                if mode == "write" {
