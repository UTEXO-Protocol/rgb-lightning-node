...
pub fn sdk_node_info() -> Result<NodeInfoV1, RlnError> {
    let state = get_uniffi_app_state()?;
            ...
            let recipients = group
                ...
                .map(|r| {
                    let assignment = match (r.assignment_kind, r.assignment_amount) {
                        (AssignmentKindV1::Fungible, Some(v)) => {
                            crate::routes::Assignment::Fungible(v)
                        }
                        (AssignmentKindV1::InflationRight, Some(v)) => {
                            crate::routes::Assignment::InflationRight(v)
                        }
                        (AssignmentKindV1::NonFungible, None) => {
                            crate::routes::Assignment::NonFungible
                        }
                        (AssignmentKindV1::ReplaceRight, None) => {
                            crate::routes::Assignment::ReplaceRight
                        }
