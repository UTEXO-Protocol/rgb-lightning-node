...
fn main() {
    ...
    // Without a node alias, ldk-node forces the "announced channels only" preference
    // (`force_announced_channel_preference`), which makes it reject RLN's private
    // (unannounced) inbound channel opens.
    builder.set_node_alias("StockLdkNode".to_string()).unwrap();
    builder
