...
impl SyncedKvStore {
    ...
    fn requires_remote_durability(primary_namespace: &str, secondary_namespace: &str) -> bool {
        (primary_namespace == crate::ldk::RGB_SENDER_FUNDING_NAMESPACE
            && secondary_namespace.is_empty())
            || (primary_namespace == crate::ldk::PSBT_NAMESPACE && secondary_namespace.is_empty())
            || (primary_namespace == crate::ldk::PENDING_FUNDING_NAMESPACE
                && secondary_namespace.is_empty())
            || (primary_namespace == lightning::rgb_utils::RGB_PRIMARY_NS
                && secondary_namespace == lightning::rgb_utils::RGB_FUNDING_ACCEPTANCE_NS)
    }
