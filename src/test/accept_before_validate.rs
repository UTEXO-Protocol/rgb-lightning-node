...
//! to `wallet.accept_transfer_consignment(...)`. That call is not a query: it
//! `store_secret_seal`s, `import_contract`s and `accept_transfer`s into the acceptor's RGB stock
...
//! ```
//!
//! Two tests drive the two halves of that, plus a baseline:
//!
//! - [`baseline_a_good_open_imports_the_contract_and_keeps_the_channel`] — what a successful
//!   colored open leaves on the acceptor, so the rejection runs can be read against it (rule 4).
//! - [`acceptor_imports_an_unrelated_consignment_before_rejecting_the_funding`] — the peer
//!   sends a valid consignment for a *different* transfer. The acceptor accepts it into its RGB
//!   stock and only then finds it says nothing about the funding output, and closes the channel.
//! - [`unsupported_assignment_variant_panics_the_acceptor`] — the assignment the acceptor
//!   finds is an `InflationRight`, which the match above has no arm for.
//!
...
//! unmodified node in both, reacting to bytes a peer chose — which is the whole point, since it is
//! the peer that builds the funding transaction and the consignment for it.
...
/// Baseline (rule 4): a successful colored open, so that the *kind* of state the rejection runs
/// leave on the acceptor can be told apart from what every open produces. The acceptor imports the
/// contract on the happy path too — the question this test asks is what is left when the consignment
/// turns out not to describe the channel.
#[serial_test::serial]
...
/// The claim: the acceptor runs `_accept_transfer` on whatever the peer sent before asking whether
/// it has anything to do with the channel.
///
/// node1 first makes a perfectly ordinary on-chain send to node3, which leaves a valid consignment
/// on its disk. It then opens a colored channel to node2 and puts *that* file on the p2p link
/// instead of the one for its funding transaction. The consignment is valid, so
/// `accept_transfer_consignment` succeeds and mutates node2's wallet; only afterwards does
/// `handle_funding` notice that it assigns nothing to the funding output.
#[serial_test::serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[traced_test]
...
/// The other half of the claim: `remote_rgb_assignments[0]` is matched with
/// `_ => unreachable!("unsupported schema")`, and `Assignment` has four variants.
///
