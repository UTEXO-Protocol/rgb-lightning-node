//! a peer-supplied `push_asset_amount` is not validated on the acceptor.
//!
//! `push_asset_amount` rides on `open_channel` as part of the RGB TLV
//! (`rgb_asset: Option<(ContractId, Option<u64>)>`, type 3 in
//! `rust-lightning/lightning/src/ln/msgs.rs:3148`). The acceptor stores it verbatim on the inbound
//! channel (`channel.rs:14008` -> `funding.rgb_asset`) and reads it back in `handle_funding`
//! (`rgb_utils/mod.rs:663-670`):
//!
//! ```text
...
//! ```
//!
//! `channel_rgb_amount` comes from the consignment the initiator sent — it is what the funding
//! output actually holds. Nothing compares the two. The only check in the codebase lives in the
//! *sender's* REST layer (`routes::open_channel`, "push_asset_amount cannot be higher than
//! asset_amount"), so an honest RLN initiator cannot produce this; the sender has to be hostile.
//!
//! The fault is injected with [`INFLATE_PUSH_ASSET_AMOUNT_ON_NODE`] /
//! [`INFLATED_PUSH_ASSET_AMOUNT`] (added for this task): the named node puts a chosen
//! `push_asset_amount` in the `open_channel` it sends while its own channel accounting keeps the
//! honest value. That is exactly the freedom any peer has — the field is peer-supplied data on the
//! wire, and the guard that would have stopped it runs on the sender, in code a hostile peer is
//! not running. Nothing is patched on the victim.
//!
//! The outcome depends on the build profile, because it hinges on an unchecked `u64` subtraction:
//! with overflow checks on (`cargo test`) the subtraction panics on the peer-message path; with
//! them off (`cargo test --release`) it wraps. Both are asserted here, selected by
//! `cfg!(debug_assertions)`.

use lightning::rgb_utils::parse_rgb_channel_info;

use crate::ldk::{INFLATED_PUSH_ASSET_AMOUNT, INFLATE_PUSH_ASSET_AMOUNT_ON_NODE};
...
/// The push a hostile peer puts on the wire: more asset than the channel holds.
...
/// Points [`INFLATE_PUSH_ASSET_AMOUNT_ON_NODE`] at a node with the value it should send, and
/// clears both on drop so a panicking test cannot leak the fault into the next one.
struct InflatedPushGuard(#[allow(dead_code)] NodeOverrideGuard);

impl InflatedPushGuard {
    fn set(node_pubkey: &str, push_asset_amount: u64) -> Self {
        *INFLATED_PUSH_ASSET_AMOUNT.lock().unwrap() = Some(push_asset_amount);
        Self(NodeOverrideGuard::set(
            &INFLATE_PUSH_ASSET_AMOUNT_ON_NODE,
            node_pubkey,
        ))
    }
}

impl Drop for InflatedPushGuard {
    fn drop(&mut self) {
        // the node override goes first: the inner `NodeOverrideGuard` only runs after this body,
        // so clearing the value first would leave a window where the node is still flagged as an
        // inflater with no value to send
        *INFLATE_PUSH_ASSET_AMOUNT_ON_NODE
            .lock()
            .unwrap_or_else(|e| e.into_inner()) = None;
        *INFLATED_PUSH_ASSET_AMOUNT
            .lock()
            .unwrap_or_else(|e| e.into_inner()) = None;
    }
}
...
/// Rule 4 baseline, and the reachability check the task asks for: the sender's REST guard is the
/// only thing that rejects a `push_asset_amount` larger than the channel's asset amount, and a
/// legal push produces the split both sides agree on.
#[serial_test::serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[traced_test]
async fn baseline_only_the_senders_rest_guard_bounds_push_asset_amount() {
    initialize();
...
/// this test itself: node1 sends an `open_channel` whose `push_asset_amount` (1000) exceeds the RGB
/// amount its consignment assigns to the funding output (600), and node2 uses it unchecked.
#[serial_test::serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[traced_test]
async fn acceptor_takes_an_inflated_push_asset_amount_from_a_hostile_peer() {
    ...
    println!(
        "\n=== node1 opens a {CHAN_ASSET_AMT}-asset channel but claims to push {HOSTILE_PUSH} ==="
    );
    let _panics = PanicCapture::install();
    let _hostile = InflatedPushGuard::set(&node1_pubkey, HOSTILE_PUSH);
    ...
    // EXPECTED AFTER FIX: the acceptor validates push_asset_amount and rejects the open cleanly,
    // never panicking under its ChannelManager locks
    assert!(
        !died,
        "an unchecked peer-supplied amount must not take the acceptor's ChannelManager down"
    );
    assert!(
    ...
    // the initiator is not the one that misbehaved from LDK's point of view: it is left holding a
    // half-open channel with its assets committed to a funding its peer can no longer answer for
    // EXPECTED AFTER FIX: a clean rejection takes neither node down
    assert!(
        channel_manager_alive(node1_addr).await,
        "the initiator survives the clean rejection"
    );
    // EXPECTED AFTER FIX: the inflated open never becomes a live channel on the initiator either —
    // the acceptor rejects it, so no channel opens ready with mis-accounted assets (and the
    // initiator is not left permanently stranded on a half-open channel its peer cannot answer for)
    ...
    // EXPECTED AFTER FIX: the initiator was never the misbehaving party — whatever RGB channel
    // info it kept stays honest (never an inflated split); the lie only ever lived on the wire
    assert!(
        node1_infos
            .iter()
    ...
    // EXPECTED AFTER FIX: the initiator's assets are not committed to an unanswerable funding —
    ...
    // EXPECTED AFTER FIX: neither build profile persists a bad split — the acceptor rejects the
    // inflated push BEFORE writing RGB channel info, so it neither underflows (debug) nor stores a
    // wrapped amount (release), and records no RGB channel info bound to the inflated open
    assert_eq!(
...
/// Overflow checks on (`cargo test`): `channel_rgb_amount - push_amount` itself panics, so the
/// acceptor dies before `write_rgb_channel_info` and binds the channel to nothing.
fn assert_overflow_panics_before_the_write(test_dir_node2: &str, temporary_channel_id: &str) {
    println!("\n--- overflow checks are on: the subtraction is what panics");
    assert!(
        PanicCapture::matching("attempt to subtract with overflow").is_some(),
        "`channel_rgb_amount - push_amount` should be the panic: {:?}",
        PanicCapture::messages()
    );
    assert!(
        PanicCapture::matching("rgb_utils/mod.rs").is_some(),
        "the panic comes from `handle_funding`: {:?}",
        PanicCapture::messages()
    );
    assert_eq!(
        rgb_amounts_on_disk(test_dir_node2, temporary_channel_id),
        None,
        "the panic is before `write_rgb_channel_info`, so the channel is bound to nothing"
    );
    assert!(
        channel_info_files(test_dir_node2).is_empty(),
        "the acceptor recorded no RGB channel info at all"
    );
}

/// Overflow checks off (`cargo test --release`): the subtraction wraps, the acceptor persists
/// asset amounts the channel cannot hold, and dies one step later when `color_commitment` asks
/// rgb-lib to assign them.
fn assert_wrapped_amount_is_persisted(node2_infos: &[(String, u64, u64)]) {
    println!("\n--- overflow checks are off: the subtraction wraps and is written to disk");
    let wrapped = CHAN_ASSET_AMT.wrapping_sub(HOSTILE_PUSH);
    assert!(
        !node2_infos.is_empty(),
        "the acceptor wrote RGB channel info before it died"
    );
    for (name, local, remote) in node2_infos {
        assert_eq!(
            (*local, *remote),
            (HOSTILE_PUSH, wrapped),
            "{name} records the pushed amount verbatim and a counterparty balance that wrapped"
        );
    }
    let (_, node2_local, node2_remote) = node2_infos[0].clone();
    assert!(
        node2_local > CHAN_ASSET_AMT,
        "the acceptor believes it holds {node2_local} of an asset the channel funds with \
         {CHAN_ASSET_AMT}"
    );
    assert!(
        node2_local.checked_add(node2_remote).is_none(),
        "the two amounts the acceptor persisted cannot even be added in u64"
    );

    // rgb-lib is what stops the wrapped value from reaching a usable channel — by panicking
    // through `color_commitment`'s `.expect("successful commitment coloring")`
    let coloring = PanicCapture::matching("greater than available");
    println!("coloring panic: {coloring:?}");
    assert!(
        coloring.is_some(),
        "rgb-lib should refuse to assign the wrapped amount: {:?}",
        PanicCapture::messages()
    );
    let coloring = coloring.unwrap();
    assert!(
        coloring.contains(&wrapped.to_string()),
        "the amount rgb-lib refused is the wrapped one: {coloring}"
    );
    assert!(
        coloring.contains(&CHAN_ASSET_AMT.to_string()),
        "against what the funding output actually holds: {coloring}"
    );
}
