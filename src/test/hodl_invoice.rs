...
async fn run_auto_claim_invoice_regression_case(node1_addr: SocketAddr, node2_addr: SocketAddr) {
    let LNInvoiceResponse { invoice } =
        ln_invoice(node2_addr, Some(HTLC_MIN_MSAT), None, None, 120).await;
...
async fn claim_hodl_invoice_btc_rgb() {
    ...
    let first_settle = claim_hodl_invoice(node2_addr, payment_hash.clone(), preimage.clone()).await;
    assert!(first_settle.changed);
    assert!(matches!(
        invoice_status(node2_addr, &invoice).await,
        InvoiceStatus::Claiming | InvoiceStatus::Succeeded
    ));
